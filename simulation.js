window.addEventListener('DOMContentLoaded', () => {
    ...
    let bodies = []; let activeEffects = []; let nextColorIndex = 0; let selectedBodyIndex = 0; let scale = 2.5e10; let offset = { x: 0, y: 0 }; let lastMousePos = { x: 0, y: 0 }; let isPanning = false;
    ...
    class NBodySimulation {
        ...
        update(targetBodies, gravityBodies) { const allGravityBodies = gravityBodies || targetBodies; const accelerations = this._calculateAccelerations(targetBodies, allGravityBodies); targetBodies.forEach((body, i) => { body.vel = body.vel.add(accelerations[i].scale(0.5 * simParams.dt)); body.pos = body.pos.add(body.vel.scale(simParams.dt)); }); const newAccelerations = this._calculateAccelerations(targetBodies, allGravityBodies); targetBodies.forEach((body, i) => { body.vel = body.vel.add(newAccelerations[i].scale(0.5 * simParams.dt)); body.acc = newAccelerations[i]; if (simParams.trailsVisible) { body.path.push(body.pos); if (body.path.length > MAX_PATH_LENGTH) body.path.shift(); } else { body.path = [body.pos]; } }); if (targetBodies === window.bodies) { this.timeElapsed += simParams.dt; } }
        _calculateAccelerations(targetBodies, gravityBodies) { const accelerations = targetBodies.map(() => new Vector()); for (let i = 0; i < targetBodies.length; i++) { for (let j = 0; j < gravityBodies.length; j++) { const target = targetBodies[i]; const source = gravityBodies[j]; if (target === source) continue; const rVec = source.pos.sub(target.pos); const rNorm = rVec.norm(); if (rNorm > 1e6) { const forceFactor = G * source.mass / Math.pow(rNorm, 3); accelerations[i] = accelerations[i].add(rVec.scale(forceFactor)); } } } return accelerations; }
    ...
    function resetSimulation() {
        ...
        const planetsData = [ { name: 'Mercury', mass: 3.3011e23, period: 0.241, eccentricity: 0.2056, color: '#A9A9A9' }, { name: 'Venus', mass: 4.8675e24, period: 0.615, eccentricity: 0.0068, color: '#F4A460' }, { name: 'Earth', mass: 5.9724e24, period: 1.0, eccentricity: 0.0167, color: '#1E90FF' }, { name: 'Mars', mass: 6.4171e23, period: 1.881, eccentricity: 0.0934, color: '#FF4500' }, { name: 'Jupiter', mass: 1.8982e27, period: 11.86, eccentricity: 0.0488, color: '#D2B48C' }, { name: 'Saturn', mass: 5.6834e26, period: 29.45, eccentricity: 0.0557, color: '#F0E68C' }, { name: 'Uranus', mass: 8.6810e25, period: 84.02, eccentricity: 0.0472, color: '#ADD8E6' }, { name: 'Neptune', mass: 1.0241e26, period: 164.8, eccentricity: 0.0086, color: '#4682B4' }, ];
        bodies = [sun];
    ...
    function init() {
        // テーブルの行を動的に管理するため、buildTableは不要
        resizeCanvas();
        setupEventListeners();
        resetSimulation();
    ...
    function updateUI() {
        ...
        while (tableBody.rows.length < bodies.length) { const row = tableBody.insertRow(); row.insertCell(); row.insertCell(); row.insertCell(); const selectCell = row.insertCell(); const radio = document.createElement('input'); radio.type = 'radio'; radio.name = 'starSelect'; selectCell.appendChild(radio); }
    ...
    // --- メインループ ---
    function animate() { if (simParams.isRunning) { for (let i = 0; i < simParams.stepsPerFrame; i++) { simulation.update(bodies); if (previewBody) { simulation.update([previewBody], bodies); } const { toRemove, toAddInfo } = simulation.handleCollisions(bodies); if (toRemove.length > 0) { bodies = bodies.filter((_, index) => !toRemove.includes(index)); toAddInfo.forEach(info => { bodies.push(info.body); activeEffects.push(new CollisionEffect(info.body.pos, info.delta_ke)); }); if (toRemove.includes(selectedBodyIndex)) { selectedBodyIndex = 0; updatePreview(); } } } } activeEffects = activeEffects.filter(effect => { effect.update(); return effect.is_active; }); draw(); updateUI(); requestAnimationFrame(animate); }
